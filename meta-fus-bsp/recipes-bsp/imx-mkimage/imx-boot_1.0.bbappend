...
# name can be defined for prefix bootloader binary
BOOT_NAME = "imx-boot"
PROVIDES = "imx-boot"
...
# don't use tagging s. uuu_bootloader_tag.bbclass
UUU_BOOTLOADER        = ""
...
do_deploy() {
    ...
    for type in ${UBOOT_CONFIG}; do
        ...
        for target in ${IMXBOOT_TARGETS}; do
            ...
            install -m 0644 ${S}/${BOOT_CONFIG_MACHINE_EXTRA}-${target} ${DEPLOYDIR}/${BOOT_CONFIG_MACHINE_EXTRA}
        done

        # The first UBOOT_CONFIG listed will be the ${BOOT_NAME} binary
        if [ ! -f "${DEPLOYDIR}/${UUU_BOOTLOADER}" ]; then
            ln -sf ${BOOT_CONFIG_MACHINE_EXTRA} ${DEPLOYDIR}/${BOOT_CONFIG_MACHINE_EXTRA}-${target}
        else
